...
public class DefaultDeployClientTest {
    @Mock private Supplier<RestOperations> restOperationsSupplier;
    @Mock private RestOperations rest;
    @Mock private DeploySettings settings;

    private DefaultDeployClient defaultDeployClient;
...
//        when(settings.getPasswords()).thenReturn(Arrays.asList("password"));
        List<DeployApplication> apps = defaultDeployClient.getApplications(instanceUrl);
...
//        when(settings.getPasswords()).thenReturn(Arrays.asList("password"));
        ...
        List<Environment> envs = defaultDeployClient.getEnvironments(apps.get(0));
...
//        when(settings.getPasswords()).thenReturn(Arrays.asList("password"));
        ...
        List<DeployEnvResCompData> data = defaultDeployClient.getEnvironmentResourceStatusData(apps.get(0), envs.get(0));
    ...
    private String getJson(String fileName) throws IOException {
        InputStream inputStream = DefaultDeployClientTest.class.getResourceAsStream(fileName);
